...
{
    printk(KERN_INFO "Mydisk: Device is open\n");
    return 0;
}
...
static int __init mydisk_init(void)
{
    ...
    mydisk_major = register_blkdev(mydisk_major, "dor");        
    ...
    sprintf(mydisk_dev.gd->disk_name,"dor");
...
static void __exit mydisk_exit(void) 
{
    ...
    vfree(dev_data);     
    unregister_blkdev(mydisk_major,"dor");
}    
